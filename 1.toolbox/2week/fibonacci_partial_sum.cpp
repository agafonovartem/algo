...
long long get_fibonacci_partial_sum_fast(long long from, long long to){
    int diff = fibonacci_sum_fast(to) - fibonacci_sum_fast(from - 1);
    if (diff < 0){
        diff = 10 + diff;
    }
    return diff;
...
int main() {
    /*
    while (true) {
        long long from = rand() % 50 + 2;
        long long to = rand() % 20 + 2 + from;
        std::cout << from << " " << to << "\n";
        long long res1 = get_fibonacci_partial_sum_naive(from, to);
        long long res2 = get_fibonacci_partial_sum_fast(from, to);
        if (res1 != res2){
    ...
    //std::cout << fibonacci_sum_fast(to) << " " << fibonacci_sum_fast(from - 1) << '\n';
